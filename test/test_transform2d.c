...
int main(void)
{
    ...
    CHECK(comp_transform2d_init(&t2d, 0.4, 0, 0.0) == 0, "init");
    CHECK(comp_transform2d_init(&t2d, 0.0, 0, 0.0) != 0, "zero threshold must be rejected");
    CHECK(comp_transform2d_init(&t2d, 0.4, 0, 0.0) == 0, "re-init");
